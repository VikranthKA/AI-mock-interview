  ...
  return mockInterviewQuestion && (
    <div className='p-5 border  rounded-lg my-10'>
        ...
        {mockInterviewQuestion.map((question,index)=>{
           return <h2 key={index} className={`p-2 bg-secondary rounded-full cursor-pointer text-xs md:text-sm text-center ${activeQuestionIndex===index ? `bg-blue-800 text-white`:``}`}>Question #{index+1}</h2>
        })} 
        </div>
        ...
        <section className='flex mb-2 gap-1 cursor-pointer' onClick={()=>textToSpeech(mockInterviewQuestion[activeQuestionIndex]?.question)}>
